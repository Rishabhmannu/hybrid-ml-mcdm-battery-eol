...
For each source S (above a row-count threshold):
  - train XGBoost SoH on all rows from sources != S
...
-----
    ...
    python scripts/cross_source_generalization.py             # full run
"""
...
def main():
    ...
    print("=" * 70)
    print(f"Cross-source LOSO  ({'SMOKE' if args.smoke else 'FULL'})"
    ...
    y_pool = np.concatenate([bundle.y_train_soh, bundle.y_val_soh, bundle.y_test_soh])
    src_pool = pd.concat([
    ...
    suffix = ("_smoke" if args.smoke else "") + (args.tag or "")
    ...
    ax.axvline(TARGETS["soh_r2"], color="#7F8C8D", linestyle="--", linewidth=0.7,
               label=f"R² = {TARGETS['soh_r2']:.2f} target")
    ...
    ax.set_title("Leave-One-Source-Out generalization — XGBoost SoH")
    ...
    n_pass = (df["r2"] >= TARGETS["soh_r2"]).sum()
    n_fail = (df["r2"] < 0).sum()
    md_path = OUT_TBL / f"findings{suffix}.md"
    md_path.write_text("\n".join([
        "# Cross-source LOSO generalization (XGBoost SoH)",
        "",
        f"_Sources held out: {len(df)}  ·  generated in {total:.1f}s_",
        "",
        "| Held-out source | n train | n test | dominant chem | R² | RMSE | MAE |",
        "|---|---|---|---|---|---|---|",
        *[f"| {r['held_out_source']} | {r['n_train']:,} | {r['n_test']:,} | "
        ...
        f"- Sources passing the 0.95 R² gate **on held-out generalization**: **{n_pass} / {len(df)}**",
        f"- Sources where R² is **negative** (worse than predicting the mean): **{n_fail} / {len(df)}**",
        "",
    ...
    print(f"\nMedian LOSO R²: {median_r2:.3f}  ·  pass: {n_pass}/{len(df)}  ·  negative: {n_fail}")
    print("Done.")
